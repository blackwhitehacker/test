<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-4">
            <a href="{{ route('inventory_requests.index', ['type' => $type]) }}" class="p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors">
    ...
    <div class="max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8" x-data="{ 
        items: [],
        init() {
            if (this.type === 'outbound' && this.items.length === 0) {
                this.addItem();
            }
        },
        addItem() {
        ...
        <form action="{{ route('inventory_requests.store', ['type' => $type]) }}" method="POST" class="space-y-8">
            @csrf
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <select name="source_type" required class="input-premium py-2 text-sm">
                            @if($type == 'inbound')
                                <option value="purchase" {{ old('source_type') == 'purchase' ? 'selected' : '' }}>Từ mua sắm mới</option>
                                <option value="transfer" {{ old('source_type') == 'transfer' ? 'selected' : '' }}>Từ điều chuyển nội bộ</option>
                            @else
                                <option value="allocation" {{ old('source_type') == 'allocation' ? 'selected' : '' }}>Cấp phát cho nhân viên</option>
                                <option value="repair" {{ old('source_type') == 'repair' ? 'selected' : '' }}>Gửi đi sửa chữa</option>
                                <option value="disposal" {{ old('source_type') == 'disposal' ? 'selected' : '' }}>Thanh lý tài sản</option>
                            @endif
                            <option value="other" {{ old('source_type') == 'other' ? 'selected' : '' }}>Khác</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-[10px] text-gray-400 italic">Hàng hóa sẽ được tự động đổ từ lô hàng này.</p>
                    </div>
                    @else
                    <div>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-8">
                    ...
                    <div x-show="items.length === 0 && !shipment_id" class="text-center py-8 bg-gray-50/50 rounded-2xl border-2 border-dashed border-gray-100 mt-4">
                        <p class="text-sm text-gray-400 italic font-medium">Bạn có thể chọn Lô hàng phía trên hoặc bấm "Thêm thủ công" để nhập danh sách hàng hóa.</p>
                    </div>
                </div>
            </div>
