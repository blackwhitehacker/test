<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('purchase_requisitions.show', $requisition) }}" class="text-gray-500 hover:text-gray-800 text-sm font-medium">
                ← Quay lại chi tiết
            </a>
        </div>
    </x-slot>
    ...
    <div class="max-w-5xl mx-auto" x-data="{ 
        ...
        <form action="{{ route('purchase_requisitions.update', $requisition) }}" method="POST" enctype="multipart/form-data" class="space-y-6 pb-20">
            @csrf
            @method('PUT')
            ...
            <div class="card-premium">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" required value="{{ old('title', $requisition->title) }}" class="enterprise-input" placeholder="Nhập tiêu đề đề xuất mua sắm...">
                    </div>
                    <div>
                        <label class="enterprise-label">Phòng ban đề xuất</label>
                        <input type="text" name="department" required value="{{ old('department', $requisition->department) }}" class="enterprise-input" placeholder="Tên phòng ban...">
                    </div>
                    <div>
                        <label class="enterprise-label">Ngày cần tài sản</label>
                        <input type="date" name="needed_date" class="enterprise-input" value="{{ old('needed_date', $requisition->needed_date) }}">
                    </div>
                    <div class="md:col-span-2" x-data="{ 
                        partnerName: '{{ old('partner_name', $requisition->partner->name ?? '') }}',
                        partners: [],
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="4" class="enterprise-input" placeholder="Chi tiết nhu cầu trang bị tài sản...">{{ old('description', $requisition->description) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="card-premium">
                ...
                <div class="flex flex-wrap gap-4 mb-8">
                    @foreach($requisition->attachments ?? [] as $file)
                        <div class="flex items-center p-3 bg-gray-50 rounded border text-sm font-medium text-gray-700">
                             <svg class="w-4 h-4 text-red-600 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg>
                             {{ $file['name'] }}
                        </div>
                    @endforeach
                </div>
                
                <h3 class="font-bold text-sm text-gray-400 uppercase mb-4 tracking-wider">Tải thêm tài liệu mới</h3>
                <div class="border-2 border-dashed border-gray-200 rounded-lg p-8 text-center bg-gray-50 group hover:border-red-400 transition-colors relative cursor-pointer">
                    ...
                    <p class="text-sm font-bold text-gray-600">Thêm tệp đính kèm mới</p>
                </div>
            </div>

            <!-- Actions -->
            <div class="flex justify-end gap-4 pt-4 border-t">
                <button type="button" @click="if(confirm('Bạn có chắc muốn xóa tờ trình này?')) document.getElementById('delete-form').submit()" class="mr-auto text-red-600 hover:text-red-800 text-sm font-bold p-2">🗑 Xóa tờ trình</button>
                <a href="{{ route('purchase_requisitions.show', $requisition) }}" class="px-6 py-2 text-sm font-bold text-gray-600 hover:text-gray-900 flex items-center">Hủy bỏ</a>
                <button type="submit" class="bg-black text-white px-12 py-3 rounded text-sm font-bold transition-all hover:bg-gray-800 active:scale-95">Lưu thay đổi</button>
            </div>
        </form>

        <form id="delete-form" action="{{ route('purchase_requisitions.destroy', $requisition) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
